...
void append(Node*&head,int data){
    ...
    if(box==NULL){head=new_node;new_node->next=head;return;}
    while(box->next!=head){box=box->next;}   //This condition establishes that the traversing element(box) has reached the first element
    box->next=new_node;
    new_node->next=head;
}
...
Line 24 i.e. while(box->next!=head){box=box->next;} establishes that the traversing element/variable has reached the first element.
